/*打开文件管理模块，负责对打开文件的管理，为用户打开文件
建立数据结构之间的勾连关系，为用户提供直接操作文件的文件描述符接口*/


//该文件改动部分内容，删去管道部分的代码，并加入初始化format代码
...
File* OpenFileTable::FAlloc()//此处无需修改
{
	int fd;
...
void InodeTable::Format()//格式化inode表
{
	Inode init;//空对象
	for (int i = 0; i < InodeTable::NINODE; ++i)
	{
